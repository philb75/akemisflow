...
export async function GET(req: NextRequest) {
  try {
    ...
    if (useSupabase) {
      ...
      suppliers = await prisma.contractor.findMany({
        orderBy: {
          createdAt: 'desc'
        }
      })
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Create contractor data with environment-aware field names and name formatting
    const baseContractorData = useSupabase ? {
    ...
    const contractorData = formatContractorNames(baseContractorData)
    ...
    if (useSupabase) {
      ...
      supplier = await prisma.contractor.create({
        data: contractorData
      })
    }
