...
export async function POST(
  ...
  try {
    ...
    console.log(`🔄 Starting single contractor Airwallex sync for ID: ${params.id}`)
    ...
    if (!result.success) {
      console.error(`❌ Sync failed for contractor ${params.id}: ${result.error}`)
      return NextResponse.json({
        success: false,
    ...
    console.log(`✅ Successfully synced contractor ${params.id}`)
