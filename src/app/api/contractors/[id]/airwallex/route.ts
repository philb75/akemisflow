...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (useSupabase) {
      ...
      airwallexData = await prisma.airwallexContractor.findFirst({
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (useSupabase) {
      ...
      // Use Prisma for local development
      result = await prisma.airwallexContractor.upsert({
        where: { beneficiaryId },
        update: {
          ...airwallexData,
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (useSupabase) {
      ...
      // Use Prisma for local development
      await prisma.airwallexContractor.deleteMany({
        where: { linkedContractorId: id }
      })
    }
