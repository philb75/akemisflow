...
interface AirwallexSupplierData {
  id?: string
...
export function SupplierComparison({ supplierId }: SupplierComparisonProps) {
  const [supplierData, setSupplierData] = useState<SupplierData | null>(null)
  const [airwallexData, setAirwallexData] = useState<AirwallexSupplierData | null>(null)
  ...
  const getAirwallexValue = (camelCase: string, snakeCase: string, data: AirwallexSupplierData | null) => {
    if (!data) return null
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      // Fetch AkemisFlow supplier data
      ...
      // Fetch Airwallex supplier data
      const airwallexResponse = await fetch(`/api/suppliers/${supplierId}/airwallex`)
  ...
  const syncToAirwallex = async () => {
    ...
    try {
      const response = await fetch(`/api/suppliers/${supplierId}/sync-to-airwallex`, {
        method: 'POST'
      })
  ...
  const syncFromAirwallex = async () => {
    ...
    try {
      const response = await fetch(`/api/suppliers/${supplierId}/sync-from-airwallex`, {
        method: 'POST'
      })
  ...
  const linkSuppliers = async () => {
    if (!airwallexData?.beneficiaryId) return
    
    try {
      const response = await fetch(`/api/suppliers/${supplierId}/link-airwallex/${airwallexData.beneficiaryId}`, {
        method: 'POST'
      })
  ...
  const isLinked = (airwallexData?.linkedSupplierId === supplierId) || (airwallexData?.linked_supplier_id === supplierId)
