/**
 * Comprehensive Contractor Data Model
...
export interface ContractorData {
  ...
  // Contract Information (Manual only)
  jobDescription?: string              // ❌ Manual only - detailed job description
  rate?: number                        // ❌ Manual only - hourly/daily rate
  rateCurrency?: string                // ❌ Manual only - rate currency (EUR, USD, etc.)
  rateFrequency?: 'HOURLY' | 'DAILY'   // ❌ Manual only - rate frequency
  
  // General Comments (Manual only)
  comments?: string                    // ❌ Manual only - general comments about contractor
...
export interface ContractorSyncOptions {
...
export interface ContractorValidation {
  isValid: boolean
  errors: string[]
  warnings: string[]
...
export const CONTRACTOR_FIELD_CATEGORIES = {
  personal: {
...
// Required fields for contractor creation
export const REQUIRED_CONTRACTOR_FIELDS = [
  'firstName',
  'lastName', 
  'email'
] as const
